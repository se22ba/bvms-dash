...
function update(job, stepDelta, extras={}){
  job.steps += (stepDelta || 0);
  
  if (job.steps >= job.total) {
    job.steps = job.total;
  }
  job.progress = Math.min(100, Math.round((job.steps / Math.max(1, job.total)) * 100));
...
app.post("/api/discover/start", async (req,res)=>{
  ...
  (async ()=>{
    try{
      ...
      update(job, 0, { state:"done", done:true, progress:100, found });
    }catch(e){
      update(job, 0, { state:"error", done:true, error:String(e) });
    }
  })();
...
app.get("/api/discover/status", (req,res)=>{
  ...
  res.json({
    ...
    processedHosts: job.processedHosts,
